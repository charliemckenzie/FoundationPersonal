...
description: 'Comprehensive web accessibility standards based on WCAG 2.2 AA, with 38+ anti-patterns, legal enforcement context (EAA, ADA Title II), WAI-ARIA patterns, and React/Next.js-specific fixes. Applied to UI surfaces (components, pages, and any .tsx) — not scripts, config, or server-only files.'
---
...
| 1.4.10 Reflow | AA | Content fits within 320px CSS width without two-dimensional scrolling. |
| 1.4.11 Non-text Contrast | AA | UI components and graphics: 3:1 against adjacent colors. |
| 1.4.12 Text Spacing | AA | No loss of content with user-overridden line-height, paragraph spacing, letter spacing, and word spacing. |
...
| 2.4.11 Focus Not Obscured | AA | Focused element not entirely hidden by other overlaying elements. *(New in 2.2)* |
| 2.5.3 Label in Name | A | Accessible name contains the label text as visually presented. |
...
> **Note:** 4.1.1 Parsing is obsolete in WCAG 2.2. Issues it covered are now addressed by 1.3.1 and 4.1.2.

---

## Legal Enforcement Context (2026)

- **European Accessibility Act (EAA)**: Enforced since June 2025 across all 27 EU member states. Fines up to EUR 3 million. References EN 301 549 (maps to WCAG 2.1 AA).
- **ADA Title II (US)**: Digital accessibility rule effective April 2026 for state/local governments. Requires WCAG 2.1 AA.
- **Section 508 (US Federal)**: References WCAG 2.0 AA.

**Target**: WCAG 2.2 AA covers all current legal requirements.
...
2. **Don't change native semantics where prohibited** — Don't add `role="heading"` to a `<button>`.
...
- **Severity**: SUGGESTION
- **Detection**: Multiple `<h1>` elements
- **WCAG**: Best practice (supports 1.3.1)

Prefer one `<h1>` per page. Use `<h2>` for sections.
...
Maintain logical nesting: `h1 > h2 > h3 > h4`. Style headings with CSS, not by choosing a different level.
...
Use CSS Grid/Flexbox for layout. If a table must be used for layout, add `role="presentation"`.
...
Do not leave focusable elements inside `aria-hidden="true"` content. Use `disabled` for native controls, remove `href`/`tabindex` for custom elements, or use `inert`.
...
```html
<!-- GOOD — content announced when injected into a pre-existing live region -->
...
- **Severity**: CRITICAL
- **Detection**: `(?:onClick|@click)` on `<div>` or `<span>` without keyboard handler
- **WCAG**: 2.1.1 (A)
...
Use native `<dialog>` with `showModal()` — it prevents keyboard focus from moving to non-dialog content and has built-in Escape key support. If a custom implementation is required: trap Tab within the dialog, dismiss on Escape, return focus to the trigger on close.
...
- **Severity**: IMPORTANT
- **Detection**: `onMouseOver|onMouseEnter` without keyboard equivalent
- **WCAG**: 2.1.1 (A)
...
Use reCAPTCHA v3 (invisible), hCaptcha accessibility mode, or alternative authentication. Never block paste or autofill on password fields.
...
Use MUI theme tokens — `theme.palette.text.primary` is calibrated to meet contrast requirements. Never use hardcoded colours.
...
Use `rem` for font sizes. Foundation requires `rem` everywhere — never `px` for font sizes.
...
Use responsive layouts (Grid, Flexbox). Avoid fixed-width containers.
...
In JSX, use `htmlFor` not `for`. The HTML attribute `for` is a reserved word in JavaScript.
...
- [ ] Touch targets at least 24x24 CSS px (WCAG 2.5.8)
