...
const meta: Meta<typeof FileUpload> = {
  title: 'Form Components / FileUpload',
...
- Selected files appear below the zone with name, size, and a remove button
...
- \`description\` — human-readable format and size hint shown inside the zone; always provide alongside \`accept\` and \`maxSizeMB\`
        `.trim(),
      },
    },
  },
...
export const WithFile: Story = {
  parameters: {
    docs: { description: { story: 'A single file has been selected. The file card appears below the zone with name, size, and a remove button.' } },
  },
  args: {
...
export const Uploading: Story = {
  parameters: {
    docs: { description: { story: 'Pass `uploadProgress` (filename → 0–100) to show a progress bar on individual file cards. The remove button is hidden while uploading.' } },
  },
  args: {
    multiple: true,
    defaultFiles: [
...
export const ErrorState: Story = {
  parameters: {
    docs: { description: { story: 'Pass `error` as a string to show an external error. Overrides any internal validation message.' } },
  },
